...
- **Golden reference (`<day>/expected_output.txt`), when present:** the run's stdout must match it. This is the only check that catches a corruption at the print CALL — an operand swapped inside a `%`-tuple, an index shifted at the print site, arithmetic wrapped around a name the self-check already pins correctly. In-file assertions are structurally blind to those (measured: 498 plants, 4 caught, and not one catch was an assertion). Opt-in by file existence, so a day without one behaves exactly as before. ⚠️ Reviewing this is a distinct job: the reference proves DRIFT, never correctness — a confidently wrong first pin is defended forever, so when a day gains its first reference, READ the output rather than the diff. And treat `--write-expected --force` on a failing day as a red flag: it is the one move that can erase a real catch, so a commit that regenerates a reference must say why the output changed on purpose. One sanctioned exception: `m01-shape-of-data/day-06-random-seeds` prints deliberately unseeded draws and must never be pinned.
