...
Have you ever played **Mad Libs** -- that game where you have a story with blank spaces
and you fill in words like "a noun" or "a funny verb" to make a silly story?

**Prompt templates** work the same way. They're pre-written prompts with **blanks
(variables)** that you fill in each time you use them. Instead of writing a brand-new
prompt from scratch every time, you create a reusable "recipe" and just swap in the
specific details.
...
Without templates, you'd rewrite the same kind of prompt over and over:
...
WITHOUT TEMPLATES (repetitive):
  ...
  ^ You're writing almost the same thing every time!
...
Not every template needs all four parts, but the more you include, the better
and more consistent your results will be.

---

## Variable Substitution

Variables are the "blanks" in your template. They're usually written with curly
braces `{like_this}`. When you use the template, you replace each variable with
a real value.
...
Here are battle-tested template patterns you can use right away:
...
A **template library** is just a collection of your best templates organized by
category. Think of it as your personal cookbook of prompt recipes.
...
You can store these as simple text files, in a spreadsheet, or in code (using
string formatting in Python, JavaScript, etc.).
...
| Vague variable names | `{x}` and `{data}` don't tell you what to fill in | Use descriptive names: `{customer_email}`, `{product_name}` |
| No output format | AI responds in unpredictable formats | Always specify the expected output format |
| Overly rigid template | Doesn't adapt to different use cases | Use optional sections and default values |
| Never testing | You don't know if the template actually works well | Test with 3+ different inputs before sharing |
...
4. **Build templates** by starting with a working prompt, identifying what changes,
   and replacing those parts with variables
...
- **Prompt engineering best practices** -- Various sources
  - OpenAI, Anthropic, and Google all publish prompt engineering guides with template
    recommendations
- **LangChain PromptTemplate documentation**
  - A popular Python library that provides a structured way to build and manage prompt
    templates programmatically

---

[Back to Techniques Overview](./README.md) | [Previous: Chain-of-Thought](./chain-of-thought.md)
