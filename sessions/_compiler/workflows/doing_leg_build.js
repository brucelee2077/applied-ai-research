...
const LEARNER_PROSE = `THE LEARNER READS THIS FILE. Everything a human sees — every print string, every section header, every comment — is lesson prose for a curious 12-YEAR-OLD for whom ENGLISH IS A SECOND LANGUAGE (user directive 2026-08-05). So: everyday words (a word learned after ~age 10 gets a plain-words parenthetical the first time — "logits (the raw scores before they become percentages)"); ONE idea per printed line; active voice; and NO idioms or dismissive asides — never "under the hood", "out of the box", "rule of thumb", "obviously", "as you can see", "trivially", "this is just". A comment above a non-obvious line says WHY in plain words, not what the syntax does. Print statements should let the reader SEE the idea land ("weights shape: (784, 128)  <- one column per hidden unit"), not just dump values. LENGTH IS NOT A DEFECT (directive 2026-08-06) — never compress an explanation to save lines; the 85-140-line target is about scope, not terseness.`
