...
const LENSES = [
  ...
  { key: 'parity', prompt: `Run: python3 sessions/_compiler/gates/lang_parity_gate.py ${source}. It is DETERMINISTIC (exit 0/6) and cannot fail open. If it prints "n/a  this day declares no Chinese yet", verdict PASS with a one-line note and emit NO findings. Otherwise report EACH FAIL line as a P0 finding kind="lang_parity": an untranslated concept, an untwinned trailing prose span, an English-only SVG label, a missing zh_ front-matter twin, a manifest topic unreachable in Chinese. The quiz answer-index mismatch is a CORRECTNESS defect — a Chinese reader told the wrong option is right — so report it as kind="correctness" P0, not as formatting. Report its "Latin word(s) not in the term whitelist" warn as a P1: either translate them or add real terms of art to sessions/_refactor/zh_terms.yaml.` },
