...
> **Naming note (two "Batch 3" files, no overlap).** This file — `batch_3_rollout_report.md` — is the **Coach Layer rollout** for the JAX & systems tier (`m07` / `m08` / `m09a` / `m09c`). The older [`batch_3_cleanup_report.md`](./batch_3_cleanup_report.md) is a **defect-only cleanup** of a *different* scope (the inference-systems tier `m06` / `m16a` / `m17a` / `m17b`) — **no module overlap**. See [`batch_3_cleanup_note.md`](./batch_3_cleanup_note.md) for the one-line disambiguation.
