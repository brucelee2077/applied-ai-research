...
#
# It also proves today's two limits: one frozen row per word, and no slot marker.
...
if __name__ == "__main__":
    ...
    # --- Part 5: limit two — nothing inside a row marks its slot ----------
    ...
    print("same SET of rows?", set(ids_a) == set(ids_b), " same ORDERED rows?",
          np.array_equal(stack_a, stack_b), " added up:", bag_a, "vs", bag_b)
    ...
    # A set cannot notice a reordering, so pin the exact id lists and the total.
    order_ok = (ids_a == [1, 5, 6] and ids_b == [6, 5, 1] and set(ids_a) == set(ids_b)
                and not np.array_equal(stack_a, stack_b) and np.array_equal(bag_b, bag_a)
                and np.array_equal(bag_a, np.array([1.3, 1.1, 0.0, 1.2])))
    claims = {"lookup": lookup_ok, "scores": scores_ok, "prediction": predict_ok,
              "size": size_ok, "frozen": bank_ok, "one-hot": onehot_ok, "order": order_ok}
