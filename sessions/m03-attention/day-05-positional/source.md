...
%%% warmup
q: Yesterday you saw one attention head has to fold every relationship into a single blend. What happens to two clean relationships when one head must do both? | a:2 | they stay perfectly sharp | one of them is dropped | they get averaged into one muddy answer | the sentence gets shorter | concept: multi-head-attention | fb: One head gives one blend, so it splits the difference (0.50/0.50) — that mush is why we run a panel of heads.
q: In multi-head attention with width d_model and h heads, how wide is each head's slice, d_k? | a:1 | d_model × h | d_model ÷ h | h ÷ d_model | always 64 | concept: multi-head-dk | fb: d_k = d_model / h — the width is split across heads, so more heads cost about the same.
q: After the heads each produce their own answer, how are they brought back into one vector per word? | a:0 | join them side by side, then mix with the matrix W_O | keep only the best head | average all the heads into one | throw the extra heads away | concept: multi-head-concat-wo | fb: Concatenate the head answers, then W_O mixes them into one final vector the next layer expects.
%%%
...
%%% hint
t1: Ask yourself one small question first — why would a single wave ever fail? Picture a slow wave and look at two seats right next to each other on it.
t2: On a slow wave, seat 5 and seat 6 sit almost at the same height, so their readings nearly match — they collide. A fast wave fixes near-neighbors but repeats, so seat 2 and seat 40 can collide instead. So try reading BOTH waves at each seat and bundling the two numbers together.
t3: Each seat's stamp is its reading from many waves at different speeds bundled together — fast waves tell nearby seats apart, slow waves tell far seats apart, so no two seats ever share the same bundle.
%%%
...
%%% viz src=../../viz/positional-collide.html title="Collide two seats on purpose" caption="Set the waves to 1 and drag the wavelength down to 4 — watch two seats end up with the SAME stamp. Now put the waves back to 4 and untick spread: still collided. Tick it again and every seat separates."
%%%
