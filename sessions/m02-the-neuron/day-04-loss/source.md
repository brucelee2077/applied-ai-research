...
%%% hint
t1: Feel the picture first, no math. The "learning push" is how hard the score shoves a weight to fix a mistake. On a yes/no call, the sigmoid output flattens out at the extreme ends — near 0 and near 1. Ask yourself: when the model is *confidently* wrong (say it screamed "0.99 spam!" but it's not spam), is it sitting on the flat part or the steep part of that squash?
t2: Work one number. MSE's push here is the plain miss times the sigmoid's steepness at that point. Confidently wrong at p = 0.99, the steepness is p × (1 − p) = 0.99 × 0.01 ≈ 0.0098 — almost zero. So the push is (miss) × 0.0098 → tiny. Cross-entropy is built so that tiny steepness factor cancels out, leaving a push equal to the plain miss (≈ 0.99) — about 100× bigger, right when you need it most.
t3: MSE + sigmoid multiplies the learning push by the sigmoid's near-zero slope exactly when the model is confidently wrong, so it barely moves (the "mud"); cross-entropy cancels that slope so the push stays strong there — that is why classifiers train on cross-entropy, not MSE.
%%%
