...
<section class="module-section" id="s1" data-sec="what">
  ...
  <div class="sec-body">
      ...
      <tr style="border-bottom:1px solid var(--line)"><td style="padding:.4rem .6rem;color:var(--ink);font-weight:600">activation function</td><td style="padding:.4rem .6rem;color:var(--ink2)">A small curve added at the very end of a neuron that bends the number it outputs.</td></tr>
      <tr style="border-bottom:1px solid var(--line)"><td style="padding:.4rem .6rem;color:var(--ink);font-weight:600">linear</td><td style="padding:.4rem .6rem;color:var(--ink2)">A straight-line relationship — like a straight ruler; stacking straights stays straight.</td></tr>
      <tr style="border-bottom:1px solid var(--line)"><td style="padding:.4rem .6rem;color:var(--ink);font-weight:600">non-linearity</td><td style="padding:.4rem .6rem;color:var(--ink2)">A bend, not a straight line — it lets stacked layers curve and combine to learn complex shapes.</td></tr>
      <tr style="border-bottom:1px solid var(--line)"><td style="padding:.4rem .6rem;color:var(--ink);font-weight:600">ReLU</td><td style="padding:.4rem .6rem;color:var(--ink2)">A one-way valve: it lets positive numbers pass and turns negatives into 0.</td></tr>
      <tr style="border-bottom:1px solid var(--line)"><td style="padding:.4rem .6rem;color:var(--ink);font-weight:600">sigmoid</td><td style="padding:.4rem .6rem;color:var(--ink2)">A soft dimmer switch that gently squashes any number into the range 0 to 1.</td></tr>
      <tr><td style="padding:.4rem .6rem;color:var(--ink);font-weight:600">saturation / dead unit</td><td style="padding:.4rem .6rem;color:var(--ink2)">When a unit's output gets stuck flat (a jammed ReLU at 0, or a sigmoid pinned near 0 or 1) so it stops learning.</td></tr>
      </tbody></table>
...
<section class="module-section" id="s4" data-sec="why">
  ...
  <div class="sec-body">
      ...
      <p>Don't take that "Slope near 0" row on faith — <b>watch the slope</b>. The dashed curve below is each activation's slope <code>f′(z)</code>; drag the marker into the flat tails and see it collapse toward <code>0</code>. That vanishing slope <em>is</em> saturation (sigmoid/tanh) and the dead-ReLU zero — the exact reason a stuck unit stops learning.</p>
      <div class="build-embed"><iframe src="../../viz/activation-derivatives.html" title="interactive: activations and their slopes" loading="lazy"></iframe><div class="cap">interactive — pick an activation, drag the input z; watch the slope flatten. <a href="../../viz/activation-derivatives.html" target="_blank" rel="noopener">open full screen</a></div></div>
      ...
      <h4>The simplest thing one line can't do: XOR</h4>
      <p>Here is that limit in one picture. A single neuron is <b>one straight line</b> (Day 1). Ask it for <span class="term" data-tip="XOR: output 1 when exactly one of two inputs is on, else 0. Its two classes sit on opposite diagonals, so no single straight line separates them.">XOR</span> — "on when exactly one input is on" — and it can never get all four cases right, because XOR is not <span class="term" data-tip="A dataset is linearly separable if one straight line (or flat plane) can put every class on its own side. XOR is the classic example that is NOT.">linearly separable</span>. Slide the line any way you like below: the best you ever manage is 3 of 4. Then tick "add a hidden layer" — two lines, composed through a non-linear activation, carve out the region a single line never could. That is exactly why depth and a bend earn their keep.</p>
      <div class="build-embed"><iframe src="../../viz/xor-limit.html" title="interactive: why one neuron can't do XOR" loading="lazy"></iframe><div class="cap">interactive — slide the single line (stuck at 3/4), then tick "add a hidden layer". <a href="../../viz/xor-limit.html" target="_blank" rel="noopener">open full screen</a></div></div>
      ...
      <p><strong>ReLU</strong> is the classic workhorse: it's dirt cheap (just a max) and it avoids the <span class="term" data-tip="When gradients shrink toward zero as they pass back through many layers, so early layers barely learn. Sigmoid suffers from this; ReLU largely avoids it.">vanishing-gradient</span> problem that plagues sigmoid in deep stacks (you'll meet gradients on Day 5). It's also why different frontier labs pick different bends: GPT-2 and GPT-3 use <b>GELU</b>, and Llama uses <b>SwiGLU</b> — smoother curves that are one real answer to the "dead neuron" risk you're about to meet below, staying slightly sloped where a plain ReLU goes flat. Sigmoid and its cousin <span class="term" data-tip="Turns a vector of scores into probabilities that sum to 1. Used at the output of a classifier.">softmax</span> live mostly at the output, where you want probabilities.</p>
