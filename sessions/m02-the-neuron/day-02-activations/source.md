...
%%% hint
t1: Don't try to hold all four layers in your head at once. Just look at ONE hand-off in the whisper picture: a person hears "1.0" and passes on "0.25." What single number did they multiply by?
t2: They multiplied by 0.25 — that's just sigmoid's steepest slope. So each layer back multiplies the signal by about 0.25. Do it four times: 1.0 × 0.25 = 0.25, then × 0.25 = 0.06, then × 0.25 = 0.016, then × 0.25 ≈ 0.004.
t3: Because each layer multiplies the learning signal by a small slope (≈0.25 for sigmoid), stacking many layers shrinks it toward zero — so the earliest layers hear almost nothing and stop learning. That fade is the vanishing gradient, and ReLU's slope of 1 is what stops it.
%%%
