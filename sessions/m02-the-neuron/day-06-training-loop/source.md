...
%%% hint
t1: Confused why a BIG step makes the loss go UP instead of just learning faster? Picture the walker in the valley again — think about what happens when one stride is longer than the distance left to the bottom.
t2: Say the bottom is 1 step away, but your stride is 3 steps long. You leap right over the bottom and land higher up the far wall — farther from the bottom than you started. Next loop the gradient sends you leaping back, past the bottom again. Each leap lands higher, so the loss climbs.
t3: The learning rate is the stride length; if it's bigger than the distance to the bottom, every nudge overshoots and the loss grows instead of falling — that's why "too big" diverges, and the fix is simply a smaller step.
%%%
