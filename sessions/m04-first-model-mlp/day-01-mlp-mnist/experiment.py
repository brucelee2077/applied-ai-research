...
if __name__ == "__main__":
    ...
    print("pre-cast floats  : min", round(float(pre_cast.min()), 1),
          "max", round(float(pre_cast.max()), 1),
          "-> rounding moved a pixel by at most", round(cast_gap, 3),
          "| still exactly black:", round(black_share * 100, 1), "%")
    print("two samples of class 0 differ by up to", round(pair_gap, 3),
    ...
    print("step 2 x@W1+b1   :", z1.shape, "min", round(float(z1.min()), 3))
    ...
    cast_ok = (float(pre_cast.min()) == 0.0 and float(pre_cast.max()) == 255.0
               and cast_gap <= 0.5)
    ...
    wobble_ok = 0.30 < black_share < 0.60 and pair_gap > 0.05
    ...
    bend_ok = float(z1.min()) < 0.0 and float(hidden.min()) == 0.0
