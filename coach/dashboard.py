...
**Level {level_num} — {level_name}** | {xp:,} XP total | {state['player']['tokens']} Tokens
"""
...
def _section_build_progress(state: dict) -> str:
    """Build-a-Transformer progress section."""
    from coach.core import BUILD_DAYS, PHASE_NAMES, get_build_progress

    progress = get_build_progress(state)
    days_done = progress["days_completed"]
    total = progress["total_days"]
    pct = progress["pct_complete"]
    components = progress["components_built"]

    bar_width = 20
    filled = round(pct / 100 * bar_width)
    bar = "#" * filled + "." * (bar_width - filled)

    lines = ["## Build Your Transformer",
             "",
             f"Progress: `[{bar}]` **{pct:.0f}%** ({days_done}/{total} days)",
             f"Components built: {len(components)}/{sum(1 for d in BUILD_DAYS.values() if d['component_id'])}",
             ""]

    for phase_num in range(1, 6):
        name = PHASE_NAMES[phase_num]
        phase_days = [d for d, info in BUILD_DAYS.items() if info["phase"] == phase_num]
        completed = set(progress["completed_days"])
        done = sum(1 for d in phase_days if d in completed)
        total_p = len(phase_days)
        status = "done" if done == total_p else f"{done}/{total_p}"
        lines.append(f"- Phase {phase_num}: {name} — {status}")
